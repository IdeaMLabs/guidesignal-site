...
(function() {
  ...
  authUtils.validateSignupForm = function() {
    ...
    if (!terms.checked) { authUtils.showError("You must accept the terms", "terms-checkbox"); return false; }
    if (!role) { authUtils.showError("Please select a role", "selected-role"); return false; }

    if (role === "student") {
      if (!document.getElementById("student-university").value) { authUtils.showError("University required", "student-university"); return false; }
      if (!document.getElementById("student-major").value) { authUtils.showError("Major required", "student-major"); return false; }
    }
    if (role === "job_seeker") {
      if (!document.getElementById("jobseeker-current-title").value) { authUtils.showError("Current title required", "jobseeker-current-title"); return false; }
      if (!document.getElementById("jobseeker-industry").value) { authUtils.showError("Industry required", "jobseeker-industry"); return false; }
    }
    if (role === "recruiter") {
      if (!document.getElementById("recruiter-company").value) { authUtils.showError("Company required", "recruiter-company"); return false; }
      if (!document.getElementById("recruiter-company-size").value) { authUtils.showError("Company size required", "recruiter-company-size"); return false; }
    }
    return true;
  };
